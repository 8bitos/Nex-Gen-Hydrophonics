...
class MonitoringNutrisi extends StatelessWidget {
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          double latestKapasitasNutrisiA =
              double.tryParse(snapshot.data?['kapasitas_nutrisi_a'] ?? '0') ??
                  0.0;
          double latestKapasitasNutrisiB =
              double.tryParse(snapshot.data?['kapasitas_nutrisi_b'] ?? '0') ??
                  0.0;
          double sensorTDS =
              double.tryParse(snapshot.data?['sensor_tds'] ?? '0') ?? 0.0;
